...
@Component({
  selector: 'app-tenant-product-settings',
  standalone: true,
...
export class TenantProductSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    console.log('⚙️ TenantProductSettingsComponent initialized');
    ...
    this.tenantService.getMyTenant().subscribe({
      next: (response) => {
        if (response.success && response.data) {
          ...
          // Update product settings with real data
