  ...
  template: `
    <div class="p-4 sm:p-6 space-y-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Product Mapping</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Assign products to tenants, branches, or plans for multi-tenant access control
          </p>
        </div>
        <button
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
          ...
          <p class="text-2xl font-bold text-orange-900 dark:text-orange-100">{{ getUniqueProducts() }}</p>
        </div>
      </div>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              @for (mapping of mappings(); track mapping.id) {
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                  ...
                  <td class="px-4 py-3">
                    <span class="text-xs font-medium text-gray-900 dark:text-white">{{ mapping.productName }}</span>
                  </td>
                  <td class="px-4 py-3">
                    <span class="text-xs font-mono text-gray-600 dark:text-gray-400">{{ mapping.productCode }}</span>
                  </td>
                  <td class="px-4 py-3">
...
export class ProductMappingComponent {
  ...
  mappings = signal<ProductMapping[]>([
    {
      id: 1,
      productId: 1,
    ...
    },
    {
      id: 2,
      productId: 2,
      productName: 'BNPL Express',
      productCode: 'BNPL-002',
      tenantId: 2,
    ...
    },
    {
      id: 3,
      productId: 3,
      productName: 'Pawn Pro',
      productCode: 'PAWN-003',
      tenantId: 3,
  ...
  getUniqueProducts(): number {
    return new Set(this.mappings().map(m => m.productId)).size;
  }
  ...
  viewAuditHistory(mapping: ProductMapping) {
    alert(`Audit History for ${mapping.productName} → ${mapping.tenantName}\n\nAssigned: ${mapping.assignedAt}\nBy: ${mapping.assignedBy}\nStatus: ${mapping.status}\nBranches: ${mapping.branches.join(', ')}`);
  }

  unmapProduct(mapping: ProductMapping) {
    if (confirm(`Are you sure you want to unmap "${mapping.productName}" from "${mapping.tenantName}"?`)) {
      alert('Product unmapped successfully! (Mock implementation)');
    }
  }

  exportMappings() {
    alert('Exporting product mappings to CSV...');
  }
}
